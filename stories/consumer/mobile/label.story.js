...
storiesOf('Consumer/Mobile/Atomic Components/Label', module)
  ...
  .add('Active Label', () => (
    <Label type={labelTypes.ACTIVE}>{sampleText}</Label>
  ))
  .add('Strong Label', () => (
    <Label type={labelTypes.STRONG}>{sampleText}</Label>
  ))
  .add('Secondary Label', () => (
    <Label type={labelTypes.SECONDARY}>{sampleText}</Label>
  ))
  .add('Clickable Label', () => (
  ...
  .add('Error Label', () => (
    <Label type={labelTypes.ERROR}>{sampleText}</Label>
  ));

...
describe('pie-chart', () => {
  ...
  it('should have label and hint', () => {
    ...
    const hintItem = wrapper.find('.pbg-mobile-hint-normal');
  ...
  it('should have correct mapped pie chart', () => {
    ...
    expect(pieChart.prop('data')[0].color).to.equal(dataPoints[0].color);
    expect(pieChart.prop('data')[0].percentage).to.equal(dataPoints[0].value);
    expect(pieChart.prop('data')[1].color).to.equal(dataPoints[1].color);

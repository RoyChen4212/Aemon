...
describe('Overlay', () => {
  it ('Should have correct class name', () => {
    const opened = true;
    const wrapper = shallow(<Overlay title="Title" opened={opened} onBackButtonClick={() => null} />);
  ...
  it('Should render the back button in header', () => {
    ...
    const wrapper = shallow(<Overlay title="Title" opened={opened} onBackButtonClick={onBackButtonClick} />);
  ...
  it('Should not break when \'title\' is not present', () => {
    const opened = false;
    const wrapper = shallow(<Overlay opened={opened} onBackButtonClick={() => null} />);

    expect(wrapper.find('.pbg-overlay-container')).to.have.lengthOf(1);
  });

  it('Should not break when \'onBackButtonClick\' is not present', () => {
    const opened = false;
    const wrapper = shallow(<Overlay opened={opened} />);
    wrapper
      .find('.pbg-overlay--header')
      .find('.pbg-mobile-label-link')
      .simulate('click');

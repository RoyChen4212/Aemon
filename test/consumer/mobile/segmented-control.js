...
describe('Segmented Control', () => {
  ...
  it('should pass label to segments', () => {
    ...
    expect(wrapper.find('.pbg-segmented-control-button').at(0).text()).to.equal(one);
    expect(wrapper.find('.pbg-segmented-control-button').at(1).text()).to.equal(two);
  });
  ...
  it('should pass type to segments', () => {
    ...
    const wrapper = shallow(<SegmentedControl segments={segments} />);
    expect(wrapper.find('.pbg-segmented-control-button').at(0).hasClass('list')).to.be.true;
    expect(wrapper.find('.pbg-segmented-control-button').at(1).hasClass('pulse')).to.be.true;
  });
  ...
  it('should call onChange with correct index upon clicking a segment', function(done) {
    ...
    const wrapper = shallow(<SegmentedControl segments={segments} onChange={onChange} />);
    wrapper.find('.pbg-segmented-control-button').at(1).simulate('click');
  });
  ...
  it('should mark as active the correct initial segment', () => {
    ...
    const wrapper = shallow(<SegmentedControl segments={segments} />);
    expect(wrapper.find('.pbg-segmented-control-button').at(1).hasClass('active')).to.be.true;
  });
  ...
  it('should change active mark upon clicking the segment', () => {
    ...
    const wrapper = shallow(<SegmentedControl segments={segments} />);
    expect(wrapper.find('.pbg-segmented-control-button').at(1).hasClass('active')).to.be.true;
    wrapper.find('.pbg-segmented-control-button').at(0).simulate('click');
    wrapper.update();
    expect(wrapper.find('.pbg-segmented-control-button').at(0).hasClass('active')).to.be.true;
  });
});
